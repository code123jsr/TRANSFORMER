...
class Utilities:
    ...
    def sanity_check(self, sentence, block_size):
        ...
        for j, attn_map in enumerate(attn_maps):
            att_map = attn_map.squeeze(0).detach()  # Remove batch dimension and convert to NumPy array

            # Check if the attention probabilities sum to 1 over rows

            for i in range(att_map.shape[0]):
              total_prob_over_rows = torch.sum(att_map[i], dim=1)
            ...
            fig, ax = plt.subplots()
            cax = ax.imshow(att_map[0].cpu().numpy(), cmap='hot', interpolation='nearest')
            ax.xaxis.tick_top()
        ...
        print("Attention maps device:", [attn_map.device for attn_map in attn_maps])
